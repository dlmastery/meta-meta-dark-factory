...
(async () => {
  ...
  await page.click('[data-panel-target="platform"]');
  ...
  await page.click('[data-panel-target="platform"]');
  await page.fill("#platformComment", "Browser reviewer confirms PB-01 local product spine is visible, but hosted auth and database runtime remain a PB-02 gap.");
  await page.click("#addPlatformComment");
  await page.waitForFunction(() => document.querySelector("#platformCommentList")?.textContent.includes("PB-02 gap"));
  await page.waitForFunction(() => document.querySelector("#platformLedgerStatus")?.textContent.includes("comments"));
  await page.click('[data-panel-target="evidence"]');
  ...
  assert(portalAfterChange.portal_control_model.recovery_batches.some((batch) => batch.id === "PB-01" && batch.status === "accepted_for_local_product_spine"), "portal should expose PB-01 local product spine after the platform panel is present");
  assert(portalAfterChange.portal_control_model.blocker_board.some((item) => item.source === "full_product_platform" && item.detail.includes("PB-02")), "portal should move the remaining blocker to hosted enterprise PB-02 after PB-01 local spine");
