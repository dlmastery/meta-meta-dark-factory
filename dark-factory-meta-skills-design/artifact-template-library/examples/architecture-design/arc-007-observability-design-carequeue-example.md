...
# ARC-007 Observability Design Worked Example
...
The `Observability Design` for CareQueue Pro records how the pilot will treat SLIs, logs, metrics. It is linked to `INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004` and remains non-authoritative until the real customer approves the project-specific version.
...
| Work ledger item | WL-CQ-ARC-007-001 |
| Accountable owner | Maya Patel, fictional clinic operations sponsor |
| Delivery owner | DFMS delivery lead |
| Review panel | SRE Lead, System Architect, Test Lead mapped to artifact-specific critic seats |
...
- SLIs: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- logs: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- metrics: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- traces: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- alerts and dashboards: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.

## Example Trace Slice

| Source | Requirement | Artifact element | Evidence | Status |
| --- | --- | --- | --- | --- |
| INT-ANS-014 | REQ-032 | SLIs | TST-ACPT-019 | example only |
| INT-ANS-021 | NFR-SEC-004 | logs | SEC-REV-006 | example only |
| DEC-CQ-003 | ARC-ADR-002 | metrics | REV-ARC-007-001 | example only |

## Example Review Notes

- Content authority critic asks whether `SLIs` is specific enough to guide downstream work.
