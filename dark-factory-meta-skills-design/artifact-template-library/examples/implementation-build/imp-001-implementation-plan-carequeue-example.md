...
# IMP-001 Implementation Plan Worked Example
...
The `Implementation Plan` for CareQueue Pro records how the pilot will treat work packages, sequencing, dependencies. It is linked to `INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004` and remains non-authoritative until the real customer approves the project-specific version.
...
| Work ledger item | WL-CQ-IMP-001-001 |
| Accountable owner | Maya Patel, fictional clinic operations sponsor |
| Delivery owner | DFMS delivery lead |
| Review panel | Implementation Lead, Delivery Manager, Test Lead mapped to artifact-specific critic seats |
...
- work packages: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- sequencing: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- dependencies: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- token estimate: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- definition of done: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.

## Example Trace Slice

| Source | Requirement | Artifact element | Evidence | Status |
| --- | --- | --- | --- | --- |
| INT-ANS-014 | REQ-032 | work packages | TST-ACPT-019 | example only |
| INT-ANS-021 | NFR-SEC-004 | sequencing | SEC-REV-006 | example only |
| DEC-CQ-003 | ARC-ADR-002 | dependencies | REV-IMP-001-001 | example only |

## Example Review Notes

- Content authority critic asks whether `work packages` is specific enough to guide downstream work.
