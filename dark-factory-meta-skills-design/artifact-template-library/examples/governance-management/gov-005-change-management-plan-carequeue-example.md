...
# GOV-005 Change Management Plan Worked Example
...
The `Change Management Plan` for CareQueue Pro records how the pilot will treat change classes, impact analysis, approval route. It is linked to `INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004` and remains non-authoritative until the real customer approves the project-specific version.
...
| Work ledger item | WL-CQ-GOV-005-001 |
| Accountable owner | Maya Patel, fictional clinic operations sponsor |
| Delivery owner | DFMS delivery lead |
| Review panel | Change Manager, Governance Auditor, Product Owner mapped to artifact-specific critic seats |
...
- change classes: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- impact analysis: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- approval route: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- token re-estimation: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- baseline update: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.

## Example Trace Slice

| Source | Requirement | Artifact element | Evidence | Status |
| --- | --- | --- | --- | --- |
| INT-ANS-014 | REQ-032 | change classes | TST-ACPT-019 | example only |
| INT-ANS-021 | NFR-SEC-004 | impact analysis | SEC-REV-006 | example only |
| DEC-CQ-003 | ARC-ADR-002 | approval route | REV-GOV-005-001 | example only |

## Example Review Notes

- Content authority critic asks whether `change classes` is specific enough to guide downstream work.
