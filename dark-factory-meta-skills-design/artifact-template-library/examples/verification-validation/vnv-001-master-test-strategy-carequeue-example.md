...
# VNV-001 Master Test Strategy Worked Example
...
The `Master Test Strategy` for CareQueue Pro records how the pilot will treat test levels, risk coverage, automation strategy. It is linked to `INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004` and remains non-authoritative until the real customer approves the project-specific version.
...
| Work ledger item | WL-CQ-VNV-001-001 |
| Accountable owner | Maya Patel, fictional clinic operations sponsor |
| Delivery owner | DFMS delivery lead |
| Review panel | Test Lead, Product Expert, Security Engineer mapped to artifact-specific critic seats |
...
- test levels: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- risk coverage: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- automation strategy: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- manual checks: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- exit criteria: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.

## Example Trace Slice

| Source | Requirement | Artifact element | Evidence | Status |
| --- | --- | --- | --- | --- |
| INT-ANS-014 | REQ-032 | test levels | TST-ACPT-019 | example only |
| INT-ANS-021 | NFR-SEC-004 | risk coverage | SEC-REV-006 | example only |
| DEC-CQ-003 | ARC-ADR-002 | automation strategy | REV-VNV-001-001 | example only |

## Example Review Notes

- Content authority critic asks whether `test levels` is specific enough to guide downstream work.
