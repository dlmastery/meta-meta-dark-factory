...
# VNV-007 Accessibility/UX Validation Worked Example
...
The `Accessibility/UX Validation` for CareQueue Pro records how the pilot will treat keyboard access, screen reader, contrast. It is linked to `INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004` and remains non-authoritative until the real customer approves the project-specific version.
...
| Work ledger item | WL-CQ-VNV-007-001 |
| Accountable owner | Maya Patel, fictional clinic operations sponsor |
| Delivery owner | DFMS delivery lead |
| Review panel | UX Expert, Product Expert, Test Lead mapped to artifact-specific critic seats |
...
- keyboard access: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- screen reader: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- contrast: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- task completion: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- state coverage: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.

## Example Trace Slice

| Source | Requirement | Artifact element | Evidence | Status |
| --- | --- | --- | --- | --- |
| INT-ANS-014 | REQ-032 | keyboard access | TST-ACPT-019 | example only |
| INT-ANS-021 | NFR-SEC-004 | screen reader | SEC-REV-006 | example only |
| DEC-CQ-003 | ARC-ADR-002 | contrast | REV-VNV-007-001 | example only |

## Example Review Notes

- Content authority critic asks whether `keyboard access` is specific enough to guide downstream work.
