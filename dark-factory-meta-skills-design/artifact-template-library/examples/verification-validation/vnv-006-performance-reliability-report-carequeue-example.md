...
# VNV-006 Performance/Reliability Report Worked Example
...
The `Performance/Reliability Report` for CareQueue Pro records how the pilot will treat load model, SLI results, capacity. It is linked to `INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004` and remains non-authoritative until the real customer approves the project-specific version.
...
| Work ledger item | WL-CQ-VNV-006-001 |
| Accountable owner | Maya Patel, fictional clinic operations sponsor |
| Delivery owner | DFMS delivery lead |
| Review panel | SRE Lead, Test Lead, System Architect mapped to artifact-specific critic seats |
...
- load model: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- SLI results: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- capacity: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- bottlenecks: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- reliability decision: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.

## Example Trace Slice

| Source | Requirement | Artifact element | Evidence | Status |
| --- | --- | --- | --- | --- |
| INT-ANS-014 | REQ-032 | load model | TST-ACPT-019 | example only |
| INT-ANS-021 | NFR-SEC-004 | SLI results | SEC-REV-006 | example only |
| DEC-CQ-003 | ARC-ADR-002 | capacity | REV-VNV-006-001 | example only |

## Example Review Notes

- Content authority critic asks whether `load model` is specific enough to guide downstream work.
