...
# EVD-003 Rubric Scorecard Worked Example
...
The `Rubric Scorecard` for CareQueue Pro records how the pilot will treat 3 reviewers, 15 checks, scores. It is linked to `INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004` and remains non-authoritative until the real customer approves the project-specific version.
...
| Work ledger item | WL-CQ-EVD-003-001 |
| Accountable owner | Maya Patel, fictional clinic operations sponsor |
| Delivery owner | DFMS delivery lead |
| Review panel | Quality Lead, Governance Auditor, Selected Experts mapped to artifact-specific critic seats |
...
- 3 reviewers: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- 15 checks: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- scores: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- failed points: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- fix evidence: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.

## Example Trace Slice

| Source | Requirement | Artifact element | Evidence | Status |
| --- | --- | --- | --- | --- |
| INT-ANS-014 | REQ-032 | 3 reviewers | TST-ACPT-019 | example only |
| INT-ANS-021 | NFR-SEC-004 | 15 checks | SEC-REV-006 | example only |
| DEC-CQ-003 | ARC-ADR-002 | scores | REV-EVD-003-001 | example only |

## Example Review Notes

- Content authority critic asks whether `3 reviewers` is specific enough to guide downstream work.
