...
# EVD-007 Refinery Gate Record Worked Example
...
The `Refinery Gate Record` for CareQueue Pro records how the pilot will treat core checks, test evidence, trace evidence. It is linked to `INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004` and remains non-authoritative until the real customer approves the project-specific version.
...
| Work ledger item | WL-CQ-EVD-007-001 |
| Accountable owner | Maya Patel, fictional clinic operations sponsor |
| Delivery owner | DFMS delivery lead |
| Review panel | Quality Lead, Test Lead, Governance Auditor mapped to artifact-specific critic seats |
...
- core checks: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- test evidence: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- trace evidence: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- rubric evidence: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- gate verdict: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.

## Example Trace Slice

| Source | Requirement | Artifact element | Evidence | Status |
| --- | --- | --- | --- | --- |
| INT-ANS-014 | REQ-032 | core checks | TST-ACPT-019 | example only |
| INT-ANS-021 | NFR-SEC-004 | test evidence | SEC-REV-006 | example only |
| DEC-CQ-003 | ARC-ADR-002 | trace evidence | REV-EVD-007-001 | example only |

## Example Review Notes

- Content authority critic asks whether `core checks` is specific enough to guide downstream work.
