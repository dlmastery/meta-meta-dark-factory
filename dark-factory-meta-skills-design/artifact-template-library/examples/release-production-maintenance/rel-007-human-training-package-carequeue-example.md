...
# REL-007 Human Training Package Worked Example
...
The `Human Training Package` for CareQueue Pro records how the pilot will treat training goals, walkthrough, exercises. It is linked to `INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004` and remains non-authoritative until the real customer approves the project-specific version.
...
| Work ledger item | WL-CQ-REL-007-001 |
| Accountable owner | Maya Patel, fictional clinic operations sponsor |
| Delivery owner | DFMS delivery lead |
| Review panel | Human Owner, Support Lead, Product Expert mapped to artifact-specific critic seats |
...
- training goals: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- walkthrough: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- exercises: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- comprehension checks: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
...
| --- | --- | --- | --- | --- |
| INT-ANS-014 | REQ-032 | training goals | TST-ACPT-019 | example only |
| INT-ANS-021 | NFR-SEC-004 | walkthrough | SEC-REV-006 | example only |
| DEC-CQ-003 | ARC-ADR-002 | exercises | REV-REL-007-001 | example only |

## Example Review Notes

- Content authority critic asks whether `training goals` is specific enough to guide downstream work.
