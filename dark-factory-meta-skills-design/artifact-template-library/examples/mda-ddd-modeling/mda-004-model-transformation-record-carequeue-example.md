...
# MDA-004 Model Transformation Record Worked Example
...
The `Model Transformation Record` for CareQueue Pro records how the pilot will treat mapping table, transformation rule, exception. It is linked to `INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004` and remains non-authoritative until the real customer approves the project-specific version.
...
| Work ledger item | WL-CQ-MDA-004-001 |
| Accountable owner | Maya Patel, fictional clinic operations sponsor |
| Delivery owner | DFMS delivery lead |
| Review panel | MDA Architect, Governance Auditor, Test Lead mapped to artifact-specific critic seats |
...
- mapping table: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- transformation rule: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- exception: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- lost information: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- verification evidence: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.

## Example Trace Slice

| Source | Requirement | Artifact element | Evidence | Status |
| --- | --- | --- | --- | --- |
| INT-ANS-014 | REQ-032 | mapping table | TST-ACPT-019 | example only |
| INT-ANS-021 | NFR-SEC-004 | transformation rule | SEC-REV-006 | example only |
| DEC-CQ-003 | ARC-ADR-002 | exception | REV-MDA-004-001 | example only |

## Example Review Notes

- Content authority critic asks whether `mapping table` is specific enough to guide downstream work.
