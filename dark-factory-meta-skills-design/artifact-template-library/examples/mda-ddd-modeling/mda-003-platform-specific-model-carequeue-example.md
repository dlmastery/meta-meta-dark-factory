...
# MDA-003 Platform-Specific Model Worked Example
...
The `Platform-Specific Model` for CareQueue Pro records how the pilot will treat runtime stack, module mapping, deployment units. It is linked to `INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004` and remains non-authoritative until the real customer approves the project-specific version.
...
| Work ledger item | WL-CQ-MDA-003-001 |
| Accountable owner | Maya Patel, fictional clinic operations sponsor |
| Delivery owner | DFMS delivery lead |
| Review panel | MDA Architect, Implementation Lead, SRE Lead mapped to artifact-specific critic seats |
...
- runtime stack: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- module mapping: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- deployment units: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- config: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- platform constraints: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.

## Example Trace Slice

| Source | Requirement | Artifact element | Evidence | Status |
| --- | --- | --- | --- | --- |
| INT-ANS-014 | REQ-032 | runtime stack | TST-ACPT-019 | example only |
| INT-ANS-021 | NFR-SEC-004 | module mapping | SEC-REV-006 | example only |
| DEC-CQ-003 | ARC-ADR-002 | deployment units | REV-MDA-003-001 | example only |

## Example Review Notes

- Content authority critic asks whether `runtime stack` is specific enough to guide downstream work.
