...
| --- | --- |
| Catalog ID | REQ-006 |
| Artifact | Assumption and Constraint Log |
| Project | Northstar Daily todo plus habits demonstrator |
| Family | Requirements and product |
| Lifecycle stage | Inception and elaboration |
| Standards basis | Professional SDLC handoff |
| Required links | Decisions, risks, open questions |
| Source coverage status before RB-08 | combined |
...
| Work ledger item | WL-NORTHSTAR-RB08-REQ-006 |
...
This artifact exists so a human reviewer can make a concrete assurance decision about assumption and constraint log for Northstar Daily without relying on a broad PRD paragraph, a dashboard card, or a future roadmap. It closes the prior combined coverage state for REQ-006 by creating a named standalone document with source evidence, trace, review, and reopening rules.
...
| Artifact coverage | project-book/38-req-006-assumption-and-constraint-log.md, project-book/records/artifact-catalog-coverage-matrix.json |

## Risks And Controls

- Risk: this artifact can become stale after product or evidence changes. Control: redo closure must reopen REQ-006 and downstream certificates.
...
| --- | --- | --- |
| 1 | Requirements Lead | Verify REQ-006 against source files, trace links, tests, and coverage matrix |
| 2 | Delivery Manager | Verify REQ-006 against source files, trace links, tests, and coverage matrix |
| 3 | Human Owner | Verify REQ-006 against source files, trace links, tests, and coverage matrix |
...
| ---: | --- | --- | --- |
| 1 | Check existence and artifact identity. | Look for missing standalone document. | Standalone REQ-006 document created. |
