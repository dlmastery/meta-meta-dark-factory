...
# ARC-006 Threat Model Worked Example
...
The `Threat Model` for CareQueue Pro records how the pilot will treat assets, actors, trust boundaries. It is linked to `INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004` and remains non-authoritative until the real customer approves the project-specific version.
...
| Work ledger item | WL-CQ-ARC-006-001 |
| Accountable owner | Maya Patel, fictional clinic operations sponsor |
| Delivery owner | DFMS delivery lead |
| Review panel | Security Engineer, System Architect, Test Lead mapped to artifact-specific critic seats |
...
- assets: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- actors: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- trust boundaries: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- abuse cases: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- mitigations: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.

## Example Trace Slice

| Source | Requirement | Artifact element | Evidence | Status |
| --- | --- | --- | --- | --- |
| INT-ANS-014 | REQ-032 | assets | TST-ACPT-019 | example only |
| INT-ANS-021 | NFR-SEC-004 | actors | SEC-REV-006 | example only |
| DEC-CQ-003 | ARC-ADR-002 | trust boundaries | REV-ARC-006-001 | example only |

## Example Review Notes

- Content authority critic asks whether `assets` is specific enough to guide downstream work.
