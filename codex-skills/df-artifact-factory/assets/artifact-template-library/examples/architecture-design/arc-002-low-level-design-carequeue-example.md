...
# ARC-002 Low-Level Design Worked Example
...
The `Low-Level Design` for CareQueue Pro records how the pilot will treat component contracts, algorithms, data structures. It is linked to `INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004` and remains non-authoritative until the real customer approves the project-specific version.
...
| Work ledger item | WL-CQ-ARC-002-001 |
| Accountable owner | Maya Patel, fictional clinic operations sponsor |
| Delivery owner | DFMS delivery lead |
| Review panel | System Architect, Implementation Lead, Test Lead mapped to artifact-specific critic seats |
...
- component contracts: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- algorithms: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- data structures: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- API details: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- test seams: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.

## Example Trace Slice

| Source | Requirement | Artifact element | Evidence | Status |
| --- | --- | --- | --- | --- |
| INT-ANS-014 | REQ-032 | component contracts | TST-ACPT-019 | example only |
| INT-ANS-021 | NFR-SEC-004 | algorithms | SEC-REV-006 | example only |
| DEC-CQ-003 | ARC-ADR-002 | data structures | REV-ARC-002-001 | example only |

## Example Review Notes

- Content authority critic asks whether `component contracts` is specific enough to guide downstream work.
