...
# GOV-008 DFMS Control Graph Worked Example
...
The `DFMS Control Graph` for CareQueue Pro records how the pilot will treat lifecycle nodes, gates, skill routing. It is linked to `INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004` and remains non-authoritative until the real customer approves the project-specific version.
...
| Work ledger item | WL-CQ-GOV-008-001 |
| Accountable owner | Maya Patel, fictional clinic operations sponsor |
| Delivery owner | DFMS delivery lead |
| Review panel | Governance Auditor, Delivery Manager, Quality Lead mapped to artifact-specific critic seats |
...
- lifecycle nodes: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- gates: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- skill routing: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- re-entry paths: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- evidence outputs: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.

## Example Trace Slice

| Source | Requirement | Artifact element | Evidence | Status |
| --- | --- | --- | --- | --- |
| INT-ANS-014 | REQ-032 | lifecycle nodes | TST-ACPT-019 | example only |
| INT-ANS-021 | NFR-SEC-004 | gates | SEC-REV-006 | example only |
| DEC-CQ-003 | ARC-ADR-002 | skill routing | REV-GOV-008-001 | example only |

## Example Review Notes

- Content authority critic asks whether `lifecycle nodes` is specific enough to guide downstream work.
