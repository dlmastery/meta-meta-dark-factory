...
# GOV-004 Risk Register Worked Example
...
The `Risk Register` for CareQueue Pro records how the pilot will treat risk triggers, probability and impact, mitigation owner. It is linked to `INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004` and remains non-authoritative until the real customer approves the project-specific version.
...
| Work ledger item | WL-CQ-GOV-004-001 |
| Accountable owner | Maya Patel, fictional clinic operations sponsor |
| Delivery owner | DFMS delivery lead |
| Review panel | Delivery Manager, Security Engineer, System Architect mapped to artifact-specific critic seats |
...
- risk triggers: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- probability and impact: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- mitigation owner: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- contingency: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- residual-risk acceptance: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.

## Example Trace Slice

| Source | Requirement | Artifact element | Evidence | Status |
| --- | --- | --- | --- | --- |
| INT-ANS-014 | REQ-032 | risk triggers | TST-ACPT-019 | example only |
| INT-ANS-021 | NFR-SEC-004 | probability and impact | SEC-REV-006 | example only |
| DEC-CQ-003 | ARC-ADR-002 | mitigation owner | REV-GOV-004-001 | example only |

## Example Review Notes

- Content authority critic asks whether `risk triggers` is specific enough to guide downstream work.
