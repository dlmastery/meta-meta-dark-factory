...
# GOV-009 Work Ledger Schema Worked Example
...
The `Work Ledger Schema` for CareQueue Pro records how the pilot will treat work item schema, durable state, owner/status. It is linked to `INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004` and remains non-authoritative until the real customer approves the project-specific version.
...
| Work ledger item | WL-CQ-GOV-009-001 |
| Accountable owner | Maya Patel, fictional clinic operations sponsor |
| Delivery owner | DFMS delivery lead |
| Review panel | Delivery Manager, Traceability Lead, Maintainer mapped to artifact-specific critic seats |
...
- work item schema: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- durable state: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- owner/status: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- evidence pointers: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- resume query fields: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.

## Example Trace Slice

| Source | Requirement | Artifact element | Evidence | Status |
| --- | --- | --- | --- | --- |
| INT-ANS-014 | REQ-032 | work item schema | TST-ACPT-019 | example only |
| INT-ANS-021 | NFR-SEC-004 | durable state | SEC-REV-006 | example only |
| DEC-CQ-003 | ARC-ADR-002 | owner/status | REV-GOV-009-001 | example only |

## Example Review Notes

- Content authority critic asks whether `work item schema` is specific enough to guide downstream work.
