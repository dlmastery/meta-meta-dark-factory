...
# GOV-006 Quality Management Plan Worked Example
...
The `Quality Management Plan` for CareQueue Pro records how the pilot will treat quality thresholds, rubric model, review cadence. It is linked to `INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004` and remains non-authoritative until the real customer approves the project-specific version.
...
| Work ledger item | WL-CQ-GOV-006-001 |
| Accountable owner | Maya Patel, fictional clinic operations sponsor |
| Delivery owner | DFMS delivery lead |
| Review panel | Quality Lead, Governance Auditor, Test Lead mapped to artifact-specific critic seats |
...
- quality thresholds: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- rubric model: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- review cadence: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- certificate policy: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- defect escape handling: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.

## Example Trace Slice

| Source | Requirement | Artifact element | Evidence | Status |
| --- | --- | --- | --- | --- |
| INT-ANS-014 | REQ-032 | quality thresholds | TST-ACPT-019 | example only |
| INT-ANS-021 | NFR-SEC-004 | rubric model | SEC-REV-006 | example only |
| DEC-CQ-003 | ARC-ADR-002 | review cadence | REV-GOV-006-001 | example only |

## Example Review Notes

- Content authority critic asks whether `quality thresholds` is specific enough to guide downstream work.
