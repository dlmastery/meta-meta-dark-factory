...
# IMP-002 Code Change Set Worked Example
...
The `Code Change Set` for CareQueue Pro records how the pilot will treat changed files, behavior change, tests. It is linked to `INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004` and remains non-authoritative until the real customer approves the project-specific version.
...
| Work ledger item | WL-CQ-IMP-002-001 |
| Accountable owner | Maya Patel, fictional clinic operations sponsor |
| Delivery owner | DFMS delivery lead |
| Review panel | Implementation Lead, Test Lead, Security Engineer mapped to artifact-specific critic seats |
...
- changed files: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- behavior change: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- tests: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- review findings: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- security impact: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.

## Example Trace Slice

| Source | Requirement | Artifact element | Evidence | Status |
| --- | --- | --- | --- | --- |
| INT-ANS-014 | REQ-032 | changed files | TST-ACPT-019 | example only |
| INT-ANS-021 | NFR-SEC-004 | behavior change | SEC-REV-006 | example only |
| DEC-CQ-003 | ARC-ADR-002 | tests | REV-IMP-002-001 | example only |

## Example Review Notes

- Content authority critic asks whether `changed files` is specific enough to guide downstream work.
