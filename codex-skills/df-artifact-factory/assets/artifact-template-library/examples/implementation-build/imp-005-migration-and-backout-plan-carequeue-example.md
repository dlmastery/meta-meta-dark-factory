...
# IMP-005 Migration and Backout Plan Worked Example
...
The `Migration and Backout Plan` for CareQueue Pro records how the pilot will treat migration runbook, dry run, validation query. It is linked to `INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004` and remains non-authoritative until the real customer approves the project-specific version.
...
| Work ledger item | WL-CQ-IMP-005-001 |
| Accountable owner | Maya Patel, fictional clinic operations sponsor |
| Delivery owner | DFMS delivery lead |
| Review panel | Data Architect, SRE Lead, Test Lead mapped to artifact-specific critic seats |
...
- migration runbook: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- dry run: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- validation query: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- rollback: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- cutover window: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.

## Example Trace Slice

| Source | Requirement | Artifact element | Evidence | Status |
| --- | --- | --- | --- | --- |
| INT-ANS-014 | REQ-032 | migration runbook | TST-ACPT-019 | example only |
| INT-ANS-021 | NFR-SEC-004 | dry run | SEC-REV-006 | example only |
| DEC-CQ-003 | ARC-ADR-002 | validation query | REV-IMP-005-001 | example only |

## Example Review Notes

- Content authority critic asks whether `migration runbook` is specific enough to guide downstream work.
