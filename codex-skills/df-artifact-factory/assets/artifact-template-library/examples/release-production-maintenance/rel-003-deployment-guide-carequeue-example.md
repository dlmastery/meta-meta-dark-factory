...
# REL-003 Deployment Guide Worked Example
...
The `Deployment Guide` for CareQueue Pro records how the pilot will treat deployment steps, prechecks, postchecks. It is linked to `INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004` and remains non-authoritative until the real customer approves the project-specific version.
...
| Work ledger item | WL-CQ-REL-003-001 |
| Accountable owner | Maya Patel, fictional clinic operations sponsor |
| Delivery owner | DFMS delivery lead |
| Review panel | SRE Lead, Security Engineer, Release Manager mapped to artifact-specific critic seats |
...
- deployment steps: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- prechecks: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- postchecks: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
...
| --- | --- | --- | --- | --- |
| INT-ANS-014 | REQ-032 | deployment steps | TST-ACPT-019 | example only |
| INT-ANS-021 | NFR-SEC-004 | prechecks | SEC-REV-006 | example only |
| DEC-CQ-003 | ARC-ADR-002 | postchecks | REV-REL-003-001 | example only |

## Example Review Notes

- Content authority critic asks whether `deployment steps` is specific enough to guide downstream work.
