...
# REL-005 Incident Response Guide Worked Example
...
The `Incident Response Guide` for CareQueue Pro records how the pilot will treat severity, incident roles, containment. It is linked to `INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004` and remains non-authoritative until the real customer approves the project-specific version.
...
| Work ledger item | WL-CQ-REL-005-001 |
| Accountable owner | Maya Patel, fictional clinic operations sponsor |
| Delivery owner | DFMS delivery lead |
| Review panel | SRE Lead, Security Engineer, Human Owner mapped to artifact-specific critic seats |
...
- incident roles: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- containment: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- communications: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- post-incident review: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.

## Example Trace Slice

| Source | Requirement | Artifact element | Evidence | Status |
| --- | --- | --- | --- | --- |
| INT-ANS-014 | REQ-032 | severity | TST-ACPT-019 | example only |
| INT-ANS-021 | NFR-SEC-004 | incident roles | SEC-REV-006 | example only |
| DEC-CQ-003 | ARC-ADR-002 | containment | REV-REL-005-001 | example only |

## Example Review Notes

- Content authority critic asks whether `severity` is specific enough to guide downstream work.
