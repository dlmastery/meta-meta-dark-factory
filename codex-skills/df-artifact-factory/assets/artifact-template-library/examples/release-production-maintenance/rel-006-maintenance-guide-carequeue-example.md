...
# REL-006 Maintenance Guide Worked Example
...
The `Maintenance Guide` for CareQueue Pro records how the pilot will treat code map, common changes, regression commands. It is linked to `INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004` and remains non-authoritative until the real customer approves the project-specific version.
...
| Work ledger item | WL-CQ-REL-006-001 |
| Accountable owner | Maya Patel, fictional clinic operations sponsor |
| Delivery owner | DFMS delivery lead |
| Review panel | Brownfield Maintainer, Implementation Lead, SRE Lead mapped to artifact-specific critic seats |
...
- code map: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- common changes: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- regression commands: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- operational tasks: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- maintenance risks: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.

## Example Trace Slice

| Source | Requirement | Artifact element | Evidence | Status |
| --- | --- | --- | --- | --- |
| INT-ANS-014 | REQ-032 | code map | TST-ACPT-019 | example only |
| INT-ANS-021 | NFR-SEC-004 | common changes | SEC-REV-006 | example only |
| DEC-CQ-003 | ARC-ADR-002 | regression commands | REV-REL-006-001 | example only |

## Example Review Notes

- Content authority critic asks whether `code map` is specific enough to guide downstream work.
