...
# EVD-006 Residual Risk Acceptance Worked Example
...
The `Residual Risk Acceptance` for CareQueue Pro records how the pilot will treat risk owner, accepted risk, mitigation. It is linked to `INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004` and remains non-authoritative until the real customer approves the project-specific version.
...
| Work ledger item | WL-CQ-EVD-006-001 |
| Accountable owner | Maya Patel, fictional clinic operations sponsor |
| Delivery owner | DFMS delivery lead |
| Review panel | Human Owner, Security Engineer, Delivery Manager mapped to artifact-specific critic seats |
...
- risk owner: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- accepted risk: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- mitigation: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- expiry: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- monitoring: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.

## Example Trace Slice

| Source | Requirement | Artifact element | Evidence | Status |
| --- | --- | --- | --- | --- |
| INT-ANS-014 | REQ-032 | risk owner | TST-ACPT-019 | example only |
| INT-ANS-021 | NFR-SEC-004 | accepted risk | SEC-REV-006 | example only |
| DEC-CQ-003 | ARC-ADR-002 | mitigation | REV-EVD-006-001 | example only |

## Example Review Notes

- Content authority critic asks whether `risk owner` is specific enough to guide downstream work.
