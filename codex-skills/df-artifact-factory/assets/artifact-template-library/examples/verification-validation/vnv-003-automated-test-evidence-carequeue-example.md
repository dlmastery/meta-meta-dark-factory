...
# VNV-003 Automated Test Evidence Worked Example
...
The `Automated Test Evidence` for CareQueue Pro records how the pilot will treat execution logs, CI run, coverage. It is linked to `INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004` and remains non-authoritative until the real customer approves the project-specific version.
...
| Work ledger item | WL-CQ-VNV-003-001 |
| Accountable owner | Maya Patel, fictional clinic operations sponsor |
| Delivery owner | DFMS delivery lead |
| Review panel | Test Lead, Implementation Lead, Quality Lead mapped to artifact-specific critic seats |
...
- execution logs: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- CI run: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- coverage: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- flake analysis: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- failure triage: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.

## Example Trace Slice

| Source | Requirement | Artifact element | Evidence | Status |
| --- | --- | --- | --- | --- |
| INT-ANS-014 | REQ-032 | execution logs | TST-ACPT-019 | example only |
| INT-ANS-021 | NFR-SEC-004 | CI run | SEC-REV-006 | example only |
| DEC-CQ-003 | ARC-ADR-002 | coverage | REV-VNV-003-001 | example only |

## Example Review Notes

- Content authority critic asks whether `execution logs` is specific enough to guide downstream work.
