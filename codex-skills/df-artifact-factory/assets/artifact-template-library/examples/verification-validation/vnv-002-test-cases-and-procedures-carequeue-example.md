...
# VNV-002 Test Cases and Procedures Worked Example
...
The `Test Cases and Procedures` for CareQueue Pro records how the pilot will treat preconditions, test steps, expected results. It is linked to `INT-ANS-014 -> REQ-032 -> ARC-API-006 -> TST-ACPT-019 -> REL-GO-004` and remains non-authoritative until the real customer approves the project-specific version.
...
| Work ledger item | WL-CQ-VNV-002-001 |
| Accountable owner | Maya Patel, fictional clinic operations sponsor |
| Delivery owner | DFMS delivery lead |
| Review panel | Test Lead, Domain Expert, Implementation Lead mapped to artifact-specific critic seats |
...
- preconditions: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- test steps: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- expected results: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- test data: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.
- rerun notes: example value recorded for the CareQueue pilot with owner, trace, evidence, and revalidation trigger.

## Example Trace Slice

| Source | Requirement | Artifact element | Evidence | Status |
| --- | --- | --- | --- | --- |
| INT-ANS-014 | REQ-032 | preconditions | TST-ACPT-019 | example only |
| INT-ANS-021 | NFR-SEC-004 | test steps | SEC-REV-006 | example only |
| DEC-CQ-003 | ARC-ADR-002 | expected results | REV-VNV-002-001 | example only |

## Example Review Notes

- Content authority critic asks whether `preconditions` is specific enough to guide downstream work.
